...
# def check_presence(word, lexicon,LEN):
...
# 			# 	continue
# 		if k == word:
# 			final_features = np.add(final_features, v)
# 	# print (final_features)
# 	return final_features

# def save_hurtlexfeats(dir_filepath, LEN,categories,s_filename,data_dict,max_num_sent, max_word_count_per_sent, embed_size):
# 	lexicon = read_lexicon('hurtlex_en.tsv',LEN,categories)
# 	for ID, sentences in enumerate(data_dict['text_sen']):
# 		feats_ID = np.zeros((max_num_sent, max_word_count_per_sent, embed_size))
# 		l = min(len(sentences), max_num_sent)
# 		for ind_sen, sent in enumerate(sentences[:l]):
# 			words = sent.split(' ')
# 			l = min(len(words), max_word_count_per_sent)
# 			for ind_w, w in enumerate(words[:l]):
# 				feats_ID[ind_sen, ind_w, :] = check_presence(w.lower(), lexicon,LEN)
# 		np.save(dir_filepath + str(ID) + '.npy', feats_ID)
...
def word_featurize(word_feats_raw, model_type, data_dict, poss_word_feats_emb_dict, use_saved_word_feats, save_word_feats, data_fold_path, save_fold_path, test_mode,filename):
	...
	for word_feat_raw_dict in word_feats_raw:
		...
		if use_saved_word_feats and os.path.isfile(di_filename):
			...
			# 		LEN = len(categories)
			# 		save_hurtlexfeats(ad_word_feats['filepath'],LEN,categories,ad_word_feats['filepath'],data_dict, data_dict['max_num_sent'], data_dict['max_words_sent'],poss_word_feats_emb_dict[word_feat_name])
