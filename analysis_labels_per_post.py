...
ours_fname = "TIST_files/bestmodel.txt"
base_fname = "TIST_files/baseline.txt"

train_coverage_fname = "TIST_files/train_coverage.txt"
samples_fname = "TIST_files/selected_samples.txt"

classwise_perf_fname = "TIST_files/classwise_performance.csv"

min_samples = 6


def build_post_dict():
    filename = 'TIST_files/cl_orig_dict.pickle' 
    if os.path.isfile(filename):
        print("loading cl_orig_dict")
        with open(filename, 'rb') as f:
            cl_orig_dict = pickle.load(f)
    else:
        task_filename = 'TIST_files/data_trans.csv'
...
def write_error_ana(dict_list, num_runs, min_num_labs, max_mum_labs, pred_freq_list, f_summary):
    ...
    for num_labs in range(min_num_labs, max_mum_labs+1):
        ...
        for n_r in range(num_runs): 
            metr_dict = calc_metrics_print(dict_list[n_r][num_labs]['pred_vals'], dict_list[n_r][num_labs]['true_vals'], metr_dict,23)
...
def get_samples(ours_fname, base_fname,train_coverage_fname, min_num_labels_thresh):
    ...
    with open("TIST_files/selected_samples.txt", 'w') as w_samples:
        ...
        for i in range(len(prop_rows)):
            ...
            t_cats = [tc_rows[c_id]['label'] for c_id in t_cats_int]
            prop_p_cats = [tc_rows[int(st.strip())]['label'] for st in prop_rows[i]['pred cats'].split(',')]
            base_p_cats = [tc_rows[int(st.strip())]['label'] for st in base_rows[i]['pred cats'].split(',')]
