...
def load_data_new(filename, data_path, save_path, test_ratio, valid_ratio, rand_state, max_words_sent, test_mode,filename_map):
...
def gen_postvecs(fname_mod,test_st,test_en,word_feats, sent_enc_feats, data_dict, model_type, num_cnn_filters, rnn_type, fname_part, loss_func, cw, nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val,stack_rnn_flag,cnn_kernel_set, save_folder_name, use_saved_model, gen_att, learn_rate, dropO1, dropO2, batch_size,epochs, save_model,st_variant,conf_scores_array,use_conf_scores,n_fine_tune_layers,tf_hub_path, output_representation, bert_trainable):
    model, model_multi_task_or_tr, att_mod, post_vec_mod= get_model(model_type, data_dict['max_post_length'], data_dict['max_num_sent'], data_dict['max_words_sent'], word_feats, sent_enc_feats, learn_rate, dropO1, dropO2, num_cnn_filters, rnn_type, loss_func, nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val, stack_rnn_flag, cnn_kernel_set, st_variant,use_conf_scores,None,None,[],[], True,n_fine_tune_layers,tf_hub_path, output_representation, bert_trainable)
...
def get_conf_scores(mod_probs,data_dict):
    conf_scores_persample = np.zeros(data_dict['NUM_CLASSES'])
...
def generate_unlab_diversity_favour(mod_op_list, data_dict, bac_map, prob_trans_type,iter,save_path,conf_dict_com,post_rep_filename):
    ...
    for pos in position_diverse:
        ...
        if conf_dict_com['gen_conf_scores']:
            conf_scores_persample = get_conf_scores(mod_op[posi],data_dict)      
...
def generate_unlab_favour(mod_op_list, data_dict, bac_map, prob_trans_type, iter,save_path,conf_dict_com):
    ...
    predict_favour_weak ={}
    favour_weak = dict.fromkeys(range(data_dict['NUM_CLASSES']),0 )
    true_vals = data_dict['lab'][:data_dict['train_en_ind']]
    trainY_list = trans_labels_multi_hot(true_vals,data_dict['NUM_CLASSES'])
        ...
        for i,pb in enumerate(mod_op):
            ...
            if count_labels >=conf_dict_com['min_num_pred_labs'] and count_labels <=conf_dict_com['max_num_pred_labs']:
                ...
                for j in pb:
                    if j >= conf_dict_com['confidence_thr'] or (1-j) >= conf_dict_com['confidence_thr']:
                        category_coun = category_coun + 1
                        if category_coun == data_dict['NUM_CLASSES']:
                            count_predicted  = count_predicted + 1
                            data_pos.append(position)
                            support_weak = []
                            overall_support = []
    ...
    y_pred_vals = di_op_to_label_lists(pred_vals,data_dict['NUM_CLASSES'])
...
def generate_unlab_diversity(mod_op_list, data_dict, bac_map, prob_trans_type,iter,save_path,conf_dict_com,post_rep_filename):
    ...
    pred_vals = di_op_to_label_lists(new_lab_list_diverse,data_dict['NUM_CLASSES'])
...
def evaluate_model_new(mod_op_list, data_dict, bac_map, prob_trans_type, metr_dict, metr_dict_perrun,thresh, att_flag, output_folder_name, fname_part_r_ind):
    ...
    pred_vals = di_op_to_label_lists(y_pred_list[0],data_dict['NUM_CLASSES'])
  
    return pred_vals, true_vals, calc_metrics_print(pred_vals, true_vals, metr_dict,data_dict['NUM_CLASSES']), calc_metrics_print(pred_vals, true_vals, metr_dict_perrun,data_dict['NUM_CLASSES'])

def sent_enc_featurize_new(sent_enc_feats_raw, model_type, data_dict, poss_sent_enc_feats_emb_dict, use_saved_sent_enc_feats, save_sent_enc_feats, data_fold_path, save_fold_path, test_mode,filename,bert_path,bert_max_seq_len,tbert_paths):
...
def prep_unlab(iter, data_dict, conf_dict_com, conf_dict_list, post_vecs,conf_scores_array,original_probs_array,initial_train_en_ind):
    for conf_dict in conf_dict_list:
        for prob_trans_type in conf_dict["prob_trans_types"]:
            trainY_list, trainY_noncat_list, num_classes_var, bac_map = transform_labels(data_dict['lab'][:data_dict['train_en_ind']], prob_trans_type, conf_dict_com['test_mode'], conf_dict_com["save_folder_name"],conf_dict_com['total_data_filename'],None,conf_dict_com['train_ratio'],data_dict['NUM_CLASSES'],conf_dict_com['filename_map_list'])
            for class_imb_flag in conf_dict["class_imb_flags"]:
                loss_func_list, nonlin, out_vec_size, cw_list = class_imb_loss_nonlin(trainY_noncat_list, class_imb_flag, num_classes_var, prob_trans_type, conf_dict_com['test_mode'], conf_dict_com["save_folder_name"],conf_dict_com['use_conf_scores'],None,None, None, None,None,[],[],conf_dict_com['uncorr_c_pairs_filename'],None,conf_dict_com['label_corr_setting'],conf_dict_com['train_ratio'],conf_dict_com['filename_map_list'])
                for model_type in conf_dict["model_types"]:
                    for word_feats_raw in conf_dict["word_feats_l"]:
                        ...
                        for sent_enc_feats_raw in conf_dict["sent_enc_feats_l"]:
                            sent_enc_feats, sent_enc_feat_str, s_filename = sent_enc_featurize_new(sent_enc_feats_raw, model_type, data_dict, conf_dict_com['poss_sent_enc_feats_emb_dict'], conf_dict_com['use_saved_sent_enc_feats'], conf_dict_com['save_sent_enc_feats'], conf_dict_com["data_folder_name"], conf_dict_com["save_folder_name"], conf_dict_com["test_mode"],conf_dict_com['total_data_filename'], conf_dict_com['bert_path'], conf_dict_com['bert_max_seq_len'], conf_dict_com['tbert_paths'])
                                                ...
                                                for att_dim in conf_dict["att_dims"]:
                                                    for stack_rnn_flag in conf_dict["stack_rnn_flags"]:
                                                        ...
                                                        for thresh in conf_dict["threshes"]:                                                            
                                                            ...
                                                            f_res.write("%s\n\n" % info_str)
                                                            print("%s\n" % info_str)
                                                            post_rep_filename = ("%s%s~%s~%d.pickle" % (conf_dict_com['save_folder_name'], 'raw_postvecs',sent_enc_feats[0]['emb'], 0))
                                                            if (conf_dict_com['st_variant'].startswith('combine') or conf_dict_com['st_variant'].startswith('diversity') or conf_dict_com['st_variant'].startswith('intersection')) and iter == 0 and not os.path.isfile(post_rep_filename):
                                                                ...
                                                                best_run_filename = ("%s%s/mod~%d~%d.h5" % (conf_dict_com['save_folder_name'],fname_part, m_ind,best_run))
                                                                post_vec_op = gen_postvecs(best_run_filename,test_st,test_en,word_feats, sent_enc_feats, data_dict, model_type, num_cnn_filters, rnn_type, fname_part, loss_func_list[0], cw_list[0], nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val,stack_rnn_flag,cnn_kernel_set, conf_dict_com["save_folder_name"], conf_dict_com["use_saved_model"], conf_dict_com["gen_att"], conf_dict_com["LEARN_RATE"], conf_dict_com["dropO1"], conf_dict_com["dropO2"], conf_dict_com["BATCH_SIZE"], conf_dict_com["EPOCHS"], conf_dict_com["save_model"],conf_dict_com['st_variant'],conf_scores_array,conf_dict_com['use_conf_scores'],conf_dict_com['n_fine_tune_layers'],conf_dict_com['bert_path'], conf_dict_com['output_representation'], conf_dict_com['bert_trainable'])
                                                            ...
                                                            if iter != conf_dict_com["num_iter"] - 1 :                                                                                                                                    
                                                                ...
                                                                data_dict_filename = ("%s%s_%s_%s_%f_%f_%f_%f_%f_%f_%d.pickle" % (conf_dict_com["save_folder_name"], 'raw_data', conf_dict_com['st_variant'], sent_enc_feats[0]['emb'],conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],conf_dict_com['hardset_thr'],conf_dict_com['hardset_perlabel_thr'], conf_dict_com['hardset_retaining_ratio'], conf_dict_com['avg_thr'], iter+1))
                                                                index_filename = ("%s%s_%s_%s_%f_%f_%f_%f_%f_%f_%d.txt" % (conf_dict_com["save_folder_name"], 'index', conf_dict_com['st_variant'],sent_enc_feats[0]['emb'],conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],conf_dict_com['hardset_thr'],conf_dict_com['hardset_perlabel_thr'], conf_dict_com['hardset_retaining_ratio'],conf_dict_com['avg_thr'],iter+1))
                                                                ...
                                                                if conf_dict_com['gen_conf_scores']:
                                                                    conf_score_filename = ("%s%s_%s_%s_%f_%f_%f_%f_%f_%f_%d.pickle" % (conf_dict_com["save_folder_name"], 'raw_conf_scores',conf_dict_com['st_variant'],sent_enc_feats[0]['emb'],conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],conf_dict_com['hardset_thr'], conf_dict_com['hardset_perlabel_thr'],conf_dict_com['hardset_retaining_ratio'],conf_dict_com['avg_thr'], iter+1))
                                                                    ...
                                                                    prob_filename = ("%s%s_%s_%s_%f_%f_%f_%f_%f_%f_%d.pickle" % (conf_dict_com["save_folder_name"], 'raw_original_probs',conf_dict_com['st_variant'],sent_enc_feats[0]['emb'],conf_dict_com['confidence_thr'], conf_dict_com['retaining_ratio'],conf_dict_com['hardset_thr'], conf_dict_com['hardset_perlabel_thr'],conf_dict_com['hardset_retaining_ratio'],conf_dict_com['avg_thr'],iter+1))
...
data_dict, data_dict_filename,cl_in_filename = load_data_new(conf_dict_com["filename"], conf_dict_com["data_folder_name"], conf_dict_com["save_folder_name"], conf_dict_com['TEST_RATIO'], conf_dict_com['VALID_RATIO'], conf_dict_com['RANDOM_STATE'], conf_dict_com['MAX_WORDS_SENT'], conf_dict_com["test_mode"],conf_dict_com['filename_map_list'])
...
conf_scores_array = np.ones([data_dict['val_en_ind'], data_dict['NUM_CLASSES']])
original_probs_array = np.zeros([0,data_dict['NUM_CLASSES']])
startTime = time.time()
...
for i in range(conf_dict_com["num_iter"]):
    ...
    if i != conf_dict_com["num_iter"] - 1 :
        ...
        for item in list_saved_folds:
            if item.endswith(".pickle") and item[0] != 'r' and item != "comb_vocab~glove~totalDat.pickle":
