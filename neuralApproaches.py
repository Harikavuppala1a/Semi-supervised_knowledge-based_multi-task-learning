...
def evaluate_model(mod_op_list, data_dict, bac_map, prob_trans_type, metr_dict, thresh, att_flag, output_folder_name, fname_part_r_ind,NUM_CLASSES):
    y_pred_list = []
    ...
    if prob_trans_type == 'lp':
        ...
        pred_vals = di_op_to_label_lists(y_pred_list[0],NUM_CLASSES)
    else:
    ...
    return pred_vals, true_vals, calc_metrics_print(pred_vals, true_vals, metr_dict, NUM_CLASSES)


def train_predict(word_feats, sent_enc_feats, trainY, data_dict, model_type, num_cnn_filters, rnn_type, fname_part, loss_func, class_w, nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val, stack_rnn_flag, cnn_kernel_set, m_ind, run_ind, save_folder_name, use_saved_model, gen_att, learn_rate, dropO1, dropO2, batch_size, num_epochs, save_model,st_variant,conf_scores_array,use_conf_scores,multi_task_tl,share_weights_sep_mt,single_inp_tasks_list,sep_inp_tasks_list,filename_map_list,map_ind,numclasses_maplist,n_fine_tune_layers,tf_hub_path, output_representation, bert_trainable,cascade,merge,slightcascade,use_knowledge, knowledge_array):
    att_op = None
    fname_mod_op = ("%s%s/iop~%d~%d.pickle" % (save_folder_name, fname_part, m_ind, run_ind))
    if use_saved_model and os.path.isfile(fname_mod_op) and map_ind == len(filename_map_list)-1:
        ...
        model, model_multi_task_or_tr, att_mod, post_vec = get_model(model_type, data_dict['max_post_length'], data_dict['max_num_sent'], data_dict['max_words_sent'], word_feats, sent_enc_feats, learn_rate, dropO1, dropO2, num_cnn_filters, rnn_type, loss_func, nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val, stack_rnn_flag, cnn_kernel_set,st_variant,use_conf_scores,multi_task_tl,share_weights_sep_mt,single_inp_tasks_list,sep_inp_tasks_list,True,n_fine_tune_layers,tf_hub_path, output_representation, bert_trainable,cascade,slightcascade,use_knowledge, knowledge_array)
        ...
        if use_saved_model and os.path.isfile(fname_mod):
            ...
            if map_ind !=0 and merge:
                fname_part_prev = fname_part.replace(filename_map_list[map_ind][:-4],filename_map_list[map_ind-1][:-4])
                fname_mod_prev = ("%s%s/mod~%d~%d.h5" % (save_folder_name, fname_part_prev, 0, 0))
                model_source, model_multi_task_or_tr_source, att_mod_source, post_vec_source = get_model(model_type, data_dict['max_post_length'], data_dict['max_num_sent'], data_dict['max_words_sent'], word_feats, sent_enc_feats, learn_rate, dropO1, dropO2, num_cnn_filters, rnn_type, loss_func, nonlin, numclasses_maplist[map_ind-1], rnn_dim, att_dim, max_pool_k_val, stack_rnn_flag, cnn_kernel_set,st_variant,use_conf_scores,multi_task_tl,share_weights_sep_mt,single_inp_tasks_list,sep_inp_tasks_list,False,n_fine_tune_layers,tf_hub_path, output_representation, bert_trainable,cascade,slightcascade,use_knowledge, knowledge_array)
                model_source.load_weights(fname_mod_prev,by_name=True)
                for i in range(len(model.layers)):
                    if model.layers[i].name== "classi":
                        break
                    model.layers[i].set_weights=model_source.layers[i].get_weights()
                    print (model.layers[i].name)
                print("model source was copied into model target") 
            if multi_task_tl ==  "tr_learn":
                training_generator_aux = TrainGenerator(np.arange(0, data_dict['train_en_ind']),trainY,word_feats,sent_enc_feats, data_dict, batch_size,conf_scores_array,use_conf_scores,multi_task_tl,True,single_inp_tasks_list,sep_inp_tasks_list,use_knowledge, knowledge_array)
            ...
            training_generator = TrainGenerator(np.arange(0, data_dict['train_en_ind']),trainY,word_feats, sent_enc_feats, data_dict, batch_size,conf_scores_array,use_conf_scores,multi_task_tl,False,single_inp_tasks_list,sep_inp_tasks_list,use_knowledge, knowledge_array)
        ...
        test_generator = TestGenerator(np.arange(data_dict['test_st_ind'], data_dict['test_en_ind']), word_feats, sent_enc_feats, data_dict, batch_size, conf_scores_array, use_conf_scores,use_knowledge, knowledge_array)
            ...
            if prob_trans_type == "di":
                ...
                for ind,sep_task_tup in enumerate(sep_inp_tasks_list):
                    ...
                    if  sep_task_name == "sepkmeans":   
                        ...
                        print (sep_task_dict['num_classes_var'])
                        cw_arr = np.empty([sep_task_dict['num_classes_var'], 2])
                        for i in range(sep_task_dict['num_classes_var']):
                            cw_arr[i] = class_weight.compute_class_weight('balanced', [0,1], sep_task_dict['trainY_noncat_list'][0][:, i])
                        sep_task_dict['cw_aux'] = cw_arr
                        sep_inp_list[ind][2] = sep_task_dict['cw_aux']
...
def transform_labels(data_trainY, prob_trans_type, test_mode, save_fold_path, filename, n_labels,train_ratio, NUM_CLASSES,map_filename):
    filename = "%slabel_info~%s~%s~%s~%s~%s.pickle" % (save_fold_path, prob_trans_type, test_mode, filename,train_ratio,map_filename)
    if os.path.isfile(filename):
        ...
        elif prob_trans_type == "multi-label":                      
            num_classes_var = n_labels
            trainY_list = [trans_labels_multi_hot_emotion(data_trainY,num_classes_var)]
            print("num of direct classes: ", num_classes_var)
            bac_map = None
            trainY_noncat_list = list(trainY_list)
        else:
